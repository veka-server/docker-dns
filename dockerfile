...
CMD echo "User: $SSH_USER, Server: $SSH_SERVER, Port: $SSH_PORT"; \
    ...
    sshpass -p $SSH_PASSWORD ssh -N -L $LOCAL_PORT:127.0.0.1:1053 -p $SSH_PORT -o StrictHostKeyChecking=no $SSH_USER@$SSH_SERVER & \
    ...
    sshpass -p $SSH_PASSWORD ssh -o StrictHostKeyChecking=no $SSH_USER@$SSH_SERVER "socat TCP-LISTEN:1053,fork UDP:127.0.0.1:$DEST_PORT"
